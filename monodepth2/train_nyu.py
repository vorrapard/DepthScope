...
from scipy.io import loadmat  # For loading NYUv2 .mat files
...
class NYUv2Dataset(Dataset):
    def __init__(self, mat_file, transform=None):
        ...
        # Load metadata (shape) to avoid keeping everything in memory
        with h5py.File(mat_file, 'r') as f:
            self.num_images = f['images'].shape[-1]  # Last dimension gives total number of images
    ...
    def __getitem__(self, idx):
        with h5py.File(self.mat_file, 'r') as f:
            ...
            rgb = f['images'][:, :, :, idx]  # Shape: (3, H, W)
            rgb = np.transpose(rgb, (1, 2, 0))  # Shape: (H, W, 3)
            rgb = np.clip(rgb * 255, 0, 255).astype(np.uint8)
            rgb_image = Image.fromarray(rgb, mode='RGB')

            # Extract ground truth depth
            depth = f['depths'][:, :, idx]  # Shape: (H, W)
            depth = Image.fromarray(depth).convert("F")  # Float32 depth map
        ...
        return {"image": rgb_image, "depth": depth}
...
def train():
    ...
    full_dataset = NYUv2Dataset(DATA_PATH, transform=transform)
    ...
    torch.save(encoder.state_dict(), os.path.join(SAVE_DIR, "encoder_finetuned.pth"))
    torch.save(decoder.state_dict(), os.path.join(SAVE_DIR, "depth_finetuned.pth"))
